...
- `JUPYTER_VERSION` — defined in `version.sh`; used for `jupyter-datascience-{arm64,amd64,s390x}`.
- `RAG_VERSION` — defined in `version.sh`; used for `rag-open-llm-s390x`. The `:${RAG_VERSION}_zdnn` variant is the research/zDNN-on build; the plain tag is the default CPU build.
...
Uses **`RAG_VERSION`** from `version.sh` (separate cadence from Jupyter’s **`JUPYTER_VERSION`** — always `source version.sh` rather than hardcoding tags).
