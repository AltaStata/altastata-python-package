...
ARM64_IMG="ghcr.io/altastata/jupyter-datascience-arm64:${VERSION}"
...
if [[ "${PUSH_JUPYTER_AMD64_TO_GHCR:-}" == "1" ]]; then
    ...
    docker buildx build --platform linux/amd64 --file containers/jupyter/Dockerfile.amd64 \
        --build-arg ALTASTATA_VERSION=${ALTASTATA_PYPI_VERSION} \
        --tag ghcr.io/altastata/jupyter-datascience-amd64:${VERSION} \
        --push .
fi
...
echo ""
echo "Images:"
echo "- ghcr.io/altastata/jupyter-datascience-arm64:${VERSION}"
if [[ "${PUSH_JUPYTER_AMD64_TO_GHCR:-}" == "1" ]]; then
    echo "- ghcr.io/altastata/jupyter-datascience-amd64:${VERSION}"
fi
echo ""

echo "Pull:"
echo "  ARM64 (Apple Silicon): docker pull ghcr.io/altastata/jupyter-datascience-arm64:${VERSION}"
if [[ "${PUSH_JUPYTER_AMD64_TO_GHCR:-}" == "1" ]]; then
    echo "  AMD64:                 docker pull ghcr.io/altastata/jupyter-datascience-amd64:${VERSION}"
fi
