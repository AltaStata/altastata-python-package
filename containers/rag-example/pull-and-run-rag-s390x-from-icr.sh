...
#
# Usage:
#   SSH_HOST=user@linuxone ACCOUNT_NAME=amazon.rsa.hpcs.youruser \
#     ICR_TOKEN=... ./containers/rag-example/pull-and-run-rag-s390x-from-icr.sh
# Required: SSH_HOST, ACCOUNT_NAME. Optional: ICR_TOKEN, SSH_KEY, REMOTE_GREP11_YAML,
#   REMOTE_HPCS_DIR, REMOTE_HPCS_BLOB, REMOTE_PROPERTIES_FILE (default: .../$ACCOUNT_NAME/my.user.properties).
#   For 8 GB VM set HF_LLM_MODEL=gpt2 to avoid OOM.
# Account dir must exist on server at $REMOTE_ALTASTATA_ACCOUNTS/$ACCOUNT_NAME.
...
#   3. *.user.properties   — REMOTE_PROPERTIES_FILE
...
: "${ACCOUNT_NAME:?Set ACCOUNT_NAME to your AltaStata account dir name}"
...
if [ "${ENABLE_ALTASTATA_CONSOLE_UI:-1}" = "1" ]; then
  ...
  echo "Then open http://localhost:${CONSOLE_UI_HOST_PORT}/ in your browser."
fi
