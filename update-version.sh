...
if [ -f "containers/confidential-gke/jupyter-deployment.yaml" ]; then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        sed -i '' "s|ghcr.io/altastata/jupyter-datascience[^:]*:[^[:space:]]*|ghcr.io/altastata/jupyter-datascience-amd64:${VERSION}|g" containers/confidential-gke/jupyter-deployment.yaml
    else
        sed -i "s|ghcr.io/altastata/jupyter-datascience[^:]*:[^[:space:]]*|ghcr.io/altastata/jupyter-datascience-amd64:${VERSION}|g" containers/confidential-gke/jupyter-deployment.yaml
    fi
